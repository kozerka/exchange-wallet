		...
		<div className={'mb-6'} key={field.name}>
			<label className={'block mb-2 text-sm font-medium text-gray-900 text-gray-800'}>
				...
				{field.type === 'select' && (
					<select
						className={
							'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-yellow-500 focus:border-yellow-500 block w-full p-2.5 '
						}
						name={field.name}
						value={value}
						onChange={e => handleInputChange(field.name, e.target.value)}
				...
				{['number', 'date'].includes(field.type) && (
					<div>
						<input
							className={
								'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  0'
							}
							type={field.type}
							name={field.name}
							value={value || ''}
			...
			{errors[field.name] && (
				<div
					className={
						'flex items-center p-4 mb-4 text-sm text-gray-600 border border-yellow-300 rounded-lg bg-yellow-50 '
					}
					role={'alert'}
				>
					<svg
